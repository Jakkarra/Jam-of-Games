...
- Added equivalence operators for HAPI_TColour and static helpers to make certain colours 
or a random colour
- cout is now routed to HAPI.DebugText and cerr as well but with error before hand and red in log
- colour now overloads insertion for use with the above
- Changed some project settings for optimisation reasons
- Changed and improved some XML calls
...
namespace HAPISPACE {
	...
	public:
		...
		// screen pointer is guaranteed to be aligned to a 16 byte boundary (128 bit)
		...
		// DEPRACATED: use cout or cerr instead
		...
		virtual bool LoadTextureAligned(const std::string &filename, BYTE** data, unsigned int &width, unsigned int &height) = 0;
